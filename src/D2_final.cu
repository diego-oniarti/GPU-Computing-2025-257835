...
__global__
void D2_final_kelner(data_t * __restrict__ vals,
        int * __restrict__ xs,
        int * __restrict__ ys, 
...
data_t* D2_final(MAT_CSR *csr, data_t *ones, int threads_per_block) {
    int ROWS = csr->nrows;
    int COLS = csr->ncols;
    ...
    for (int r=-PRERUNS; r<RUNS; r++) {
        ...
        D2_final_kelner<<<n_blocks, threads_per_block>>>(vals, xs, ys, vec, ret, COLS, ROWS);
